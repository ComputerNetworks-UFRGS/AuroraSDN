function initDownPacketsChart(){
    ...
    var svg = d3.select("#down_packets_chart").append("svg")
    ...
    var old_obj = {}
    var old_time = 0
    var downPacketsInterval = setInterval(updateDownPacketsChart, $( "#chart_update_selector" ).val()*1000);
    function updateDownPacketsChart() {
        console.log('***Down Packets Chart Sync***');
        sendpack = []
        modstate = []
        req = []
        ...
        var new_obj = {'date': parseDate(getTimeNow()),
                  'Send-Packet': dataset['controller']['custom']['aggregate']['control_messages_count']['send_packet_count'].toString(),
                  'Modify-State': dataset['controller']['custom']['aggregate']['control_messages_count']['modify_state_count'].toString(),
                  'Read-State Req': dataset['controller']['custom']['aggregate']['control_messages_count']['read_state_request_count'].toString()}
        ...
        if (data.length > 1){
            ...
            if (new_time !== old_time){
                var send_packet_rate = ((new_obj['Send-Packet'] - old_obj['Send-Packet']) / difference_time) * 100;
                var modify_state_rate = ((new_obj['Modify-State'] - old_obj['Modify-State']) / difference_time ) * 100;
                var read_state_req_rate = ((new_obj['Read-State Req'] - old_obj['Read-State Req']) / difference_time) * 100;
        ...
        var svg = d3.select("#down_packets_chart").transition();
...
//                        return color(d.name);
        ...
        d3.select("#down_packets_chart").selectAll('.browser').select("text")
            .datum(function(d) {
                return {name: d.name, value: d.values[d.values.length - 1]};
            })
            .attr("transform", function(d) {
        ...
        svg.select(".y.axis") // change the y axis
            .duration(750)
            .call(yAxis);
        clearInterval(downPacketsInterval);
        downPacketsInterval = setInterval(updateDownPacketsChart, $( "#chart_update_selector" ).val()*1000);
