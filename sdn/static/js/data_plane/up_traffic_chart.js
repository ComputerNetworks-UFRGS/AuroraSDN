function initUpTrafficChart(){
    ...
    var svg = d3.select("#up_traffic_chart").append("svg")
    ...
    var old_obj = {}
    var old_time = 0
    var upTrafficInterval = setInterval(updateUpTrafficChart, $( "#chart_update_selector" ).val()*1000);
    function updateUpTrafficChart() {
        console.log('***Up Traffic Chart Sync***');
        packin = []
        reply = []
        ...
        var new_obj = {'date': parseDate(getTimeNow()),
                  'Packet-In': dataset['controller']['custom']['aggregate']['control_messages_count']['packet_in_bytes'].toString(),
                  'Read-State Reply': dataset['controller']['custom']['aggregate']['control_messages_count']['read_state_reply_bytes'].toString()}
        //console.log('new_obj');
        //console.log(new_obj);
        ...
        if (data.length > 1){
            ...
            if (new_time !== old_time){
                var packet_in_rate = ((new_obj['Packet-In'] - old_obj['Packet-In']) / difference_time * 8 / 1000) * 100;
                var read_state_reply_rate = ((new_obj['Read-State Reply'] - old_obj['Read-State Reply']) / difference_time * 8 / 1000) * 100;
                //console.log('packet_in');
        ...
        var svg = d3.select("#up_traffic_chart").transition();
...
//                        return color(d.name);
        ...
        d3.select("#up_traffic_chart").selectAll('.browser').select("text")
            .datum(function(d) {
                return {name: d.name, value: d.values[d.values.length - 1]};
            })
            .attr("transform", function(d) {
        ...
        svg.select(".y.axis") // change the y axis
            .duration(750)
            .call(yAxis);
        clearInterval(upTrafficInterval);
        upTrafficInterval = setInterval(updateUpTrafficChart, $( "#chart_update_selector" ).val()*1000);
